  ...
  <Disclosure as="nav" class="bg-indigo-600 dark:bg-zinc-800" v-slot="{ open }">
      ...
      <div class="flex justify-between h-16">
        <div class="flex">
          ...
          <div class="hidden md:ml-6 md:flex md:items-center md:space-x-4">
            <Link v-for="item in navigation" :key="item.name" :href="item.href" :class="[item.current ? 'bg-indigo-700 dark:bg-zinc-900 text-white' : 'text-white hover:bg-indigo-500 hover:bg-opacity-75 dark:hover:bg-opacity-100 dark:text-gray-300 dark:hover:bg-zinc-700 dark:hover:text-white', 'px-3 py-2 rounded-md text-sm font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
          </div>
        </div>
        <div class="flex items-center">
          ...
          <div class="hidden md:ml-4 md:flex-shrink-0 md:flex md:items-center">
            ...
            <Menu as="div" class="relative ml-3">
              <div>
                <MenuButton class="flex text-sm rounded-full bg-zinc-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white">
              ...
              <transition enter-active-class="transition duration-200 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                <MenuItems class="absolute right-0 w-48 py-1 mt-2 origin-top-right bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                  <MenuItem v-for="item in userNavigation" :key="item.name" v-slot="{ active }">
                    <Link :href="item.href" :method="item.method" :class="[active ? 'bg-zinc-100' : '', 'block px-4 py-2 text-sm text-gray-700']">{{ item.name }}</Link>
                  </MenuItem>
                </MenuItems>
              </transition>
            </Menu>
    ...
    <DisclosurePanel class="md:hidden">
      <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
        <Link v-for="item in navigation" :key="item.name" as="a" :href="item.href" :method="item.method" :class="[item.current ? 'bg-indigo-700 dark:bg-zinc-900 text-white' : 'text-white hover:bg-indigo-500 hover:bg-opacity-75 dark:hover:bg-opacity-100 dark:text-gray-300 dark:hover:bg-zinc-700 dark:hover:text-white', 'block px-3 py-2 rounded-md text-base font-medium']" :aria-current="item.current ? 'page' : undefined">{{ item.name }}</Link>
      </div>
      <div class="pt-4 pb-3 border-t border-gray-700">
        ...
        <div class="px-2 mt-3 space-y-1 sm:px-3">
          <Link v-for="item in userNavigation" :key="item.name" as="a" :href="item.href" :method="item.method" class="block px-3 py-2 text-base font-medium text-white rounded-md hover:bg-indigo-500 hover:bg-opacity-75 dark:hover:bg-opacity-100 dark:text-gray-400 hover:text-white dark:hover:bg-zinc-700">{{ item.name }}</Link>
        </div>
      </div>
    </DisclosurePanel>
  </Disclosure>
